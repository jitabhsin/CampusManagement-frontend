...
import { validateUser } from "../../Services/LoginService"; // Keep this
...
const LoginForm = () => {
      ...
      .then((response) => {
        ...
        if (userRole && typeof userRole === 'string') {
            ...
            if (normalizedRole === "ADMIN") {
              ...
              console.error("Login succeeded but received unknown role string:", userRole);
              setApiError("Login successful, but your assigned role is unrecognized.");
            }
        } else {
          // Role property was missing, null, or not a string in the response
          console.error("Login succeeded but role information was missing or invalid in response:", response.data);
          setApiError("Login successful, but role information is missing or invalid.");
        }
        // -------------------------
      })
      .catch((error) => { // Axios errors have a 'response' property
  ...
  return (
    ...
    <div className="relative w-full max-w-md p-8 space-y-6 bg-black/40 backdrop-blur-2xl rounded-2xl shadow-2xl overflow-visible">
       ...
       {/* Form */}
       <form method="post" className="space-y-6" onSubmit={handleValidation}> {/* Ensure onSubmit is correct */}
         {/* Username */}
         <div>
